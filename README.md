...
-
##### 前言：
第一次自己独立写一个github工程,完全是出于兴趣和想要提高自己的心.	email:  LandGrey@qq.com.
...
##### 2. 支持使用数字、小写字母与大写字母两两组合的任意位数爆破字典生成
  例：
       `python pydictor -t dL --len 2 4`			生成数字和小写字母组成的所有2-4位长度字典

##### 3. 支持使用数字、小写字母与大写字母3者组合的任意位数爆破字典生成
  例：
       `python pydictor -t dLc --len 4 6`		生成数字、小写字母和大写字母组成的所有4-6位字典
...
##### 5. 支持使用自定义字符串、字符生成所有可能性组合的字典
  例:
	   `python pydictor -cm abc ABC 123 .`			生成由'abc'、'ABC'、'123' 和'.'生成的所有可能性组合字典
	   
  注:  当需要空格等特殊字符时,请加双引号单独包围特殊字符,如:abc " " 123 asdf;不支持指定生成此类字典的长度.
